...
async function MediaContainer(){
    const jsonFile = await fetch('JSONS/Shop/media.json');
...
// Big Panel cars are for trends.
// Regular panel cards are for shop items.
async function BigPanelGrid() {
    const jsonFile = await fetch('JSONS/Shop/bigcards.json');
    const jsonString = await jsonFile.text();
    const data = JSON.parse(jsonString);

    const panelGrid = document.getElementsByClassName("bigPanelGrid")
     ...
     for(let i = 0; i < panelGrid.length; i++) {
        ...
        card.innerHTML = BigPanelCard(element.title,element.description, element.image, "260vw", "260vw")
        panelGrid[i].appendChild(card)
        })
    }      
}

function BigPanelCard(title,info,file,width,height){
    return `
...
async function ItemGrid() {
    const jsonFile = await fetch('JSONS/Shop/itemscards.json');
    const jsonString = await jsonFile.text();
    const data = JSON.parse(jsonString);

    const panelGrid = document.getElementsByClassName("itemGrid")
     ...
     for(let i = 0; i < panelGrid.length; i++) {
        ...
        let card = document.createElement("div")
        card.classList.add("panelCard")
        card.innerHTML = ItemCard(element.item,element.price, element.image, "130vw", "130vw")
        panelGrid[i].appendChild(card)
        })
    }      
}

function ItemCard(info,price,file,width,height){
    return `
            <img src=${file} width=${width} height=${height}></img>
            <p>${info}</p>
            <p>${price}</p>
...
async function FilterGrid() {
    const jsonFile = await fetch('JSONS/Shop/filtercards.json');
    const jsonString = await jsonFile.text();
    const data = JSON.parse(jsonString);

    const panelGrid = document.getElementsByClassName("filterGrid")
     ...
     for(let i = 0; i < panelGrid.length; i++) {
        ...
        let card = document.createElement("div")
        card.classList.add("panelCard")
        card.innerHTML = FilterCard(element.item, element.image, "80vw", "80vw")
        panelGrid[i].appendChild(card)
        })
    }      
}
...
function FilterCard(info,file,width,height){
    return `
            <img src=${file} width=${width} height=${height}></img>
            <p>${info}</p>
            `
...
async function PanelGrid() {
    const jsonFile = await fetch('JSONS/Shop/cards.json');
    const jsonString = await jsonFile.text();
    const data = JSON.parse(jsonString);
    ...
    const panelGrid = document.getElementsByClassName("panelGrid")
     ...
     for(let i = 0; i < panelGrid.length; i++) {
        ...
        let card = document.createElement("div")
        card.classList.add("panelCard")
        card.innerHTML = PanelCard(element.title,element.description, element.image, "170vw", "170vw")
        panelGrid[i].appendChild(card)
        })
    }      
}

function PanelCard(title,info, file,width,height){
    return `
            <p>${title}</p>
            <img src=${file} width=${width} height=${height}></img>
            <p>${info}</p>`
