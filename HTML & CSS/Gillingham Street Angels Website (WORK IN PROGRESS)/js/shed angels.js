...
async function MediaContainer(){
    const jsonFile = await fetch('JSONS/Home/media.json');
...
async function PanelGrid() {
    const jsonFile = await fetch('JSONS/Home/cards.json');
    const jsonString = await jsonFile.text();
    const data = JSON.parse(jsonString);
    ...
    const panelGrid = document.getElementsByClassName("panelGrid")
     ...
     for(let i = 0; i < panelGrid.length; i++) {
        ...
        card.innerHTML = PanelCard(element.description, element.image, "170vw", "170vw")
        panelGrid[i].appendChild(card)
        })
    }      
}

function PanelCard(info, file,width,height){
    return `
